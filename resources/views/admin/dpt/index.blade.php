            ...
            <x-card.card>
                <div class="row">
                    <div class="col-md-12 mb-3">
                        <a href="{{ url('admin/dpt/tambah') }}" class="btn btn-sm btn-outline-success float-end">
                    ...
                    <tbody>
                        @foreach ($list as $dpt)
                                ...
                                <td>
                                    <center>
                                        <x-button.btndefault class="warning" icons="fa-eye"
                                            url="admin/dpt/detail/{{ $dpt->id }}" />
                                        <x-button.btndefault class="primary" icons="fa-edit"
                                            url="admin/dpt/edit/{{ $dpt->id }}" />
                            ...
                            <x-modal.modaldelete id="delete{{ $dpt->id }}"
                                action="{{ url('admin/dpt/hapus', $dpt->id) }}" method="POST" />
                        @endforeach
                    </tbody>
                </table>
            </x-card.card>
